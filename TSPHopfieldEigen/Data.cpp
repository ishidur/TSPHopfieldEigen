...
double kronecker_delta(int i, int j)
{
	if (j < 0)
	{
		j = static_cast<int>(cities.size() - 1);
	}
...
{
	int n = static_cast<int>(cities.size() * cities.size());
	Eigen::MatrixXd tmp = Eigen::MatrixXd::Constant(n, n, 0.0);
	for (int s0 = 0; s0 < n; ++s0)
	{
		int x = static_cast<int>(s0 / cities.size());
		int i = static_cast<int>(s0 % cities.size());
		for (int s1 = 0; s1 < n; ++s1)
		{
			int y = static_cast<int>(s1 / cities.size());
			int j = static_cast<int>(s1 % cities.size());
